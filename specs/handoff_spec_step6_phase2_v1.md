...
**闸门 A — decodability 在新模型上重现。** 在目标模型激活上重跑 Phase 1 的 probe(diff-of-means / logistic,GroupKFold by scenario),确认 order 在中层可解码(找到**解码峰值层 L\***)。**direction 必须从 L\*(中层)取,不从 layer 0**——避开 BPE 表面伪信号。

**闸门 B — 行为把手存在(unpatched)。** 定义
`order_score = logP(C_concl | persona+Q') − logP(C_reas | persona+Q')`(teacher-forcing 两个续写各跑一次,取续写 token 的 logprob 之和)。
- 对 **conclusion-first persona**,unpatched `order_score` 应 **> 0**;对 **reasons-first persona**,应 **< 0**。
- 报告两组 `order_score` 的分布 + 分离度(across held-out scenarios,带误差棒)。
- **若 unpatched 行为把手不存在**(两组 order_score 不分离)→ 模型行为上没用这个偏好,patching 无的放矢 → 如实报告并停(这本身是结论:可解码但行为上不体现)。
